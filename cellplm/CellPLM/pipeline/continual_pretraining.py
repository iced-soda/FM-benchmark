...
ContinualPretrainingDefaultPipelineConfig = {
    'lr': 2e-4, # Same as pretraining (1e-4 for Nicheformer)
    'wd': 1e-8, # Same as pretraining (1e-3 for Nicheformer)
    'scheduler': 'plat',
    'epochs': 2,
    'max_eval_batch_size': 100000,
    'patience': 5, # Same as imputation task 
    'workers': 4,
}

class ContinualPretrainingPipeline(Pipeline):
    def __init__(self,
                 pretrain_prefix: str,
                 overwrite_config,
    ...
    def fit(self, adata: ad.AnnData,
        ...
        config = ContinualPretrainingDefaultPipelineConfig.copy()
        if train_config:
            config.update(train_config)
        
        # Load pretrained model
        self.model = load_pretrain(self.pretrain_directory, self.pretrain_prefix)

        # Set up optimizer and learning rate scheduler
        optimizer = torch.optim.Adam(self.model.parameters(), lr=train_config['lr'], weight_decay=config['wd'])
        
        if config['scheduler'] == 'plat':
            scheduler = ReduceLROnPlateau(optimizer, 'min', patience=config['patience'], factor=0.9)
        else:
            scheduler = None
        ...
        dataloader = DataLoader(dataset, batch_size=train_config.get('batch_size', 64), shuffle=True, num_workers=0)
        ...
        for epoch in range(train_config['max_epochs']):
            ...
            for i, data_dict in enumerate(dataloader):
                ...
                input_dict = {key: data_dict[key].to(device) for key in data_dict if key != 'gene_list'}
