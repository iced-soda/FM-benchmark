...
def manual_train_fm(config=None):
    ...
    wandb_logger = WandbLogger(project=f'FM-{config["organ"]}', entity='sallych2311-unsw')

    checkpoint_path = '/media/rokny/DATA3/Sally/Nicheformer/nicheformer/checkpoints'
    
    checkpoint_callback = ModelCheckpoint(dirpath=checkpoint_path, every_n_train_steps=1500, monitor='train_loss', save_top_k=-1)
    ...
    trainer = pl.Trainer(
                        ...
                        default_root_dir=f'/media/rokny/DATA3/Sally/Nicheformer/nicheformer/train_logs/trained_model_heads_{config["nheads"]}_blocks_{config["nlayers"]}/',
