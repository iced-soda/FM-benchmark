...
class NicheformerFineTune(pl.LightningModule):
    ...
    def forward(self, x: Tensor, attention_mask: Tensor, pert_flags: Optional[Tensor] = None) -> dict[str, Tensor]:
    ...
    def predict_step(self, batch: dict[str, Tensor], batch_idx: int) -> Tensor:
        ...
        true_labels = batch.get("celltype_codes", None)

        return predictions, cls_prediction, true_labels
