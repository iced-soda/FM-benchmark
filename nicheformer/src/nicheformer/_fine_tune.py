...
def fine_tune_predictions(config=None):
    ...
    # print("Keys in model's state_dict:")
    # for key in state_dict_keys:
    #     print(key)

    # checkpoint = torch.load('/media/rokny/DATA3/Sally/Nicheformer/nicheformer/nicheformer.ckpt', map_location='cpu', weights_only=False)
    ...
    run_name += f"_{config['checkpoint_path'].split('.')[0]}_steps"
    
    if config['baseline']:
        run_name += "_baseline"
    
    wandb_logger = WandbLogger(project=f"Get_predictions_{config['organ']}", entity="sallych2311-unsw", name=run_name)
    
    checkpoint_callback = ModelCheckpoint(dirpath="./fine_tuned_models", every_n_train_steps=5000, monitor='fine_tuning_regression_train', save_top_k=-1)
    ...
    if config['organ'] == 'entire_mouse':
        ...
        splits=True    
    if config['organ'] == 'xenium_lung':
        path_organ = '/media/rokny/DATA3/Sally/Nicheformer/nicheformer/data/tokenized/xenium_lung'
        label_key = ['assay', 'specie', 'modality', 'idx', 'X', 'density_0']
        splits=True
